...
@Component({
  selector: 'app-due-job',
  templateUrl: './due-job.component.html',
  styleUrls: ['./due-job.component.scss']
})
export class DueJobComponent implements OnInit {
vessel
title=""
status=0
unscheduledJob:any;
  ...
  eqpCode
    checkVesselIsSelected(){
      ...
      if(this.vesselName!=null && this.eqpCode!=null){
        this.router.navigate([`landing//dueJob/${this.vesselName}/${this.eqpCode}`]);
      }
       this.route.params.subscribe(params => {
         ...
         // alert(this.vesselId)
         this.router.navigate([`landing/dueJob/${this.vesselId}`]);
        }
     });
